...
\author{Antoine Sognos, STE Polytech Montpellier}	
\maketitle
...
\item Panels (b) and (c) show, respectively, the number of clusters and the noise share for each $(\varepsilon_s,\text{MinPts})$. The optimum is around \fbox{$\varepsilon_s = 40$~km, $\text{MinPts} = 7$}, which maximizes coherent clusters ($\approx$ 55) while keeping noise near \($\approx$ 23\%\).
\end{enumerate}
...
\section*{Results: detection and characterization of flash droughts}
